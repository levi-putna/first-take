...
export function assertChromiumHint(): void {
  try {
    ...
    throw new Error(
      "playwright is not installed. From this repo run: yarn workspace @levi-putna/storyboard-renderer exec playwright install chromium. After npx, run: npx playwright install chromium",
    );
  }
}
